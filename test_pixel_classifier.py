...
from utils.data_util import tma_4096_crop_class_printname, tma_4096_palette
...
def test_one_classifier(model_path, data_loader):
    ...
    #--- Define at which pixel batch index images end ---#
    ...
    with torch.no_grad():
        ...
        for idx, (data, ground_truth) in enumerate(data_loader):
            #--- Get model predictions on batch of pixel features ---#
...
def main():
    ...
    #--- Call helper function to get test set predictions for model ---#
    split_name = "test"
    ground_truth_mask_list, predicted_mask_list = test_one_classifier(
        os.path.join(training_run_dir, training_run, "model_name.pth"), test_dataloader)

    #--- Save predicted test masks ---#
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment', type=str,
                        default="/path/to/Histopathology_Dataset_GAN/experiments/TMA_4096_tile.json")

    opts = parser.parse_args()
    args = json.load(open(opts.experiment, 'r'))

    training_run_dir = "/path/to/Histopathology_Dataset_GAN/training-runs/"
    training_run = "experiment_name_to_test"
    SAVE_PATH = os.path.join(training_run_dir, training_run, "test_mask_pred")

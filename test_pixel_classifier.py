...
from utils.data_util import tma_4096_crop_class, tma_4096_palette, tma_4096_image_idxs
...
def test_one_classifier(model_path, data_loader):
    ...
    with torch.no_grad():
        ground_truth_mask = torch.zeros((4096, 4096))
        predicted_mask = torch.zeros((4096, 4096))
        ...
        for idx, (data, ground_truth) in enumerate(data_loader):
            ...
            ground_truth_mask[idx % 4096] = ground_truth
            predicted_mask[idx % 4096] = pixel_preds
    ...
    for i in range(len(tma_4096_crop_class)):
        log_string(logger, tma_4096_crop_class[i] + " Accuracy: " + str(round(class_accuracies[i], 5)))
    log_string(logger, "")
...
def main():
    ...
    for idx in range(len(predicted_mask_list)):
        print("Saving predicted mask and ground truth mask for image", tma_4096_image_idxs[30 + idx])
        # all_classifiers_pred_mask_list[idx] is (1, 1, 4096, 4096)
        plot_img_and_colored_mask(predicted_mask_list[idx], ground_truth_mask_list[idx], tma_4096_image_idxs[30 + idx], split_name)

...
def plot_img_and_colored_mask(mask, ground_truth_img, image_num, SAVE_PATH, split):
    ...
    plt.savefig(os.path.join(SAVE_PATH, split + "_image" + str(image_num) + "_ensemble_pred_mask.png"))
    plt.clf()

    plt.imshow(colorize_ground_truth)
    plt.savefig(os.path.join(SAVE_PATH, split + "_image" + str(image_num) + "_ensemble_ground_truth_mask.png"))
    plt.clf()
    plt.close()
...
def main():
    ...
    # print("Length of test dataset: " + str(len(test_set)) + "\n")

    train_dataloader = DataLoader(training_set, batch_size=1024, shuffle=False)
    val_dataloader = DataLoader(validation_set, batch_size=1024, shuffle=False)
...
if __name__ == "__main__":
    ...
    training_run = "0011-TMA_Arteriole_stylegan2_ada"
